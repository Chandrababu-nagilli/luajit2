...
[root@my-build-vm-watsonx-data luajit2]# cat README.md 
# Name
...
* [Build, install, and test on Linux](#build-install-and-test-on-linux)
...
Install a recent C toolchain and GNU Make. On Debian/Ubuntu, the usual minimum
is the `build-essential` package.

For running the repository test suite under `t/`, install Perl plus `prove`
from Perl's core tooling. The CI configuration also installs these Perl modules
...
To run the external OpenResty LuaJIT test suite, clone:
...
Build LuaJIT from the repository root:

```bash
make
```

Build with a custom installation prefix:

```bash
make PREFIX=/absolute/prefix
```

Useful optional build flags documented in this repository include:

```bash
make XCFLAGS=-DLUAJIT_DISABLE_GC64
make XCFLAGS=-DLUAJIT_DISABLE_JIT
make XCFLAGS=-DLUAJIT_DISABLE_FFI
make XCFLAGS=-DLUA_USE_TRACE_LOGS
```
...
```bash
make install PREFIX=/absolute/prefix
```

If you build with `PREFIX=...`, use the same `PREFIX` again for `make install`.

## Test

Run the in-repository test suite:

```bash
prove -I. t
```

Run the external OpenResty LuaJIT test suite against an installed prefix:

```bash
cd ../luajit2-test-suite
./run-tests -j 1 /usr/local
```

If LuaJIT was installed to a custom prefix, pass that prefix instead of
`/usr/local`.

## CI-style test flow used by this repository

The CI configuration in `.travis.yml` uses this sequence:

```bash
git clone https://github.com/openresty/luajit2-test-suite.git ../luajit2-test-suite
/usr/bin/env perl $(command -v cpanm) --sudo --notest IPC::Run3 Test::Base Test::LongString Parallel::ForkManager
make -j${JOBS:-3} CCDEBUG=-g Q= PREFIX=${LUAJIT_PREFIX:-/opt/luajit21} CC=${CC:-gcc} XCFLAGS="${LUAJIT_XCFLAGS}"
sudo make install PREFIX=${LUAJIT_PREFIX:-/opt/luajit21}
PATH=${LUAJIT_PREFIX:-/opt/luajit21}/bin:$PATH prove -I. t
cd ../luajit2-test-suite
./run-tests -j 1 ${LUAJIT_PREFIX:-/opt/luajit21}
```
